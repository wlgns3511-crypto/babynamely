  ...
  return (
    <>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntityOfPage: {
              "@type": "WebPage",
              "@id": `https://nameblooms.com/blog/${slug}/`,
            },
          }),
        }}
      />
      ...
      <nav className="text-xs text-slate-500 mb-6">
        ...
        <a href="/blog/" className="hover:text-purple-600">Guides</a>
        <span className="mx-1">›</span>
      ...
      <header className="mb-8">
        ...
        <div className="text-xs text-slate-400 flex items-center gap-3">
          ...
          <span>· NameBlooms Editorial Team</span>
        </div>
      </header>
      ...
      <div className="mt-12 p-6 bg-purple-50 border border-purple-200 rounded-xl">
        ...
        <p className="text-slate-600 text-sm mb-4">
          Use our free tools to browse thousands of baby names with meanings, origins, and popularity trends since 1880.
        </p>
        <div className="flex flex-wrap gap-3">
          <a
            href="/names/gender/boy/"
            className="text-sm px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 font-medium"
          >
            Boy Names
          </a>
          <a
            href="/names/gender/girl/"
            className="text-sm px-4 py-2 bg-white border border-purple-300 text-purple-700 rounded-lg hover:bg-purple-50 font-medium"
          >
            Girl Names
          </a>
          <a
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {suggestions.map((p) => (
              <a
                key={p.slug}
                href={`/blog/${p.slug}/`}
                className="block border border-slate-200 rounded-xl p-4 hover:border-purple-300 hover:shadow-sm transition-all group"
              >
                <span className="text-xs text-purple-600">{p.category}</span>
                <h3 className="text-sm font-semibold text-slate-900 group-hover:text-purple-700 mt-1 leading-snug">
                  {p.title}
                </h3>
